...
import h5py
'''
import os  # 处理文件路径（比如找文件、建文件夹）
from collections import namedtuple, defaultdict  # 特殊数据结构：namedtuple是带名字的元组（方便存数据），defaultdict是带默认值的字典
from torch.utils.data import DataLoader, Dataset  # PyTorch核心：Dataset是自定义数据集的基类，DataLoader用来批量取数据
import numpy as np  # 数值计算（处理向量/矩阵）
from tqdm import tqdm  # 显示进度条（比如加载数据时看进度）
import torch, torch.nn as nn  # PyTorch深度学习核心
import h5py  # 读取HDF5格式文件（预训练词向量常用这个格式）
